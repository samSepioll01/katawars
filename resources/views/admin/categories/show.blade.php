<x-app-layout>
    <x-layout.wrapped-admin-sections>
        ...
        <div class="flex flex-col lg:flex-row w-full justify-end items-center px-5 py-8">
            ...
            <div class="py-5">
                <form action="{{ route('admin.categories.edit', $category) }}" method="get" class="px-5 py-2">
                    <x-jet-button class="w-32 flex justify-center">Edit</x-jet-button>
                </form>
            </div>

            <div class="py-5">
                <form action="{{ route('admin.categories.destroy', $category) }}" method="post" class="px-5 py-2">
                    @csrf
                    @method('DELETE')
            ...
            <div class="lg:w-1/2">
                <table class="my-10">
                    <thead class="text-gray-800 text-2xl font-semibold w-full">
                        <tr class="w-full">
                            <div class="text-3xl font-semibold flex justify-start">{{ ucwords($category->name) }}</div>
                        </tr>
                    </thead>
                    <tbody class="">
                        <tr>
                            <td class="pr-10">Name</td>
                            <td class="py-5">{{ ucwords($category->name) }}</td>
                        </tr>
                        <tr>
                            <td class="pr-10">Background Color</td>
                            <td class="py-5">{!! $category->bg_color !!}</td>
                        </tr>
                        <tr>
                            <td>Created At</td>
