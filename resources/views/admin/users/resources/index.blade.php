    ...
    <x-layout.wrapped-admin-sections>
        ...
        <div class="w-full" x-data="{showDeleteBTN: false}">
            <div class="py-3 flex flex-col">
                <div>
                    <h1 class="text-gray-800 text-2xl font-semibold">Resources Published</h1>
                </div>
                ...
                <div class="py-5 w-full flex justify-end">
                    ...
                    <x-jet-button id="deleteBTN" class="w-32 flex justify-center" x-ref="deletebtn" x-show="showDeleteBTN" style="display: none;"
                        x-on:click="
                            ...
                            axios({
                                method: 'post',
                                url: '{{ route('users.resources.destroy-multiple', ['user' => $user]) }}',
                                responseType: 'json',
                                data: {
                                    ids: ids,
                                }
                    ...
                    <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
                        <table class="min-w-full leading-normal">
                            ...
                            >
                                    ...
                                    @foreach ($resources as $resource )
                                        <tr>
                                            ...
                                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <a href="{{ route('users.resources.show', ['user' => $user, 'resource' => $resource]) }}" class="flex items-center">
                                                    <input type="checkbox" name="resources[]" class="mr-2" id="{{ $resource->id }}">
                                                    <span>{{ $resource->title }}</span>
                                                </a>
                                            </td>

                                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <a href="{{ $resource->url }}" class="">
                                                    {{ $resource->url }}
                                                </a>
                                            </td>
                                            ...
                                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <a href="{{ $resource->kata->challenge->url }}">
                                                    {{ $resource->kata->challenge->title }}
                                                </a>
                                ...
                                @else
                                <tr>
                                    <td colspan="6" class="py-4 text-xl font-semibold text-center">No resources founded.</td>
                                </tr>
